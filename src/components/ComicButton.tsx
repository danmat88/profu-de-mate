...
export function ComicButton({ title, subtitle, icon, trailingIcon = 'next', tone = 'lime', compact = false, onPress, style }: Props) {
  ...
  return (
    <View style={[styles.wrap, compact && styles.wrapCompact, style]}>
      ...
      <Animated.View style={{ transform: [{ translateY: press.interpolate({ inputRange: [0, 1], outputRange: [0, 5] }) }, { scale: press.interpolate({ inputRange: [0, 1], outputRange: [1, 0.985] }) }] }}>
        <Pressable
          accessibilityRole="button"
          onPressIn={() => move(1)}
          onPressOut={() => move(0)}
          onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); onPress(); }}
          style={[styles.face, compact && styles.faceCompact, { backgroundColor: fills[tone] }]}
        >
          ...
          {icon ? (
            <View style={styles.icon}><AppIcon name={icon} size={compact ? 46 : 58} /></View>
          ) : null}
          <View style={styles.copy}>
            <Text style={[styles.title, compact && styles.titleCompact, light && styles.light]}>{title}</Text>
            {subtitle ? <Text style={[styles.subtitle, light && styles.lightSoft]}>{subtitle}</Text> : null}
          </View>
          {trailingIcon ? <MiniGlyph name={trailingIcon} size={compact ? 21 : 27} color={light ? colors.paper : colors.ink} /> : null}
        </Pressable>
      </Animated.View>
    </View>
  );
...
const styles = StyleSheet.create({
  wrap: { minHeight: 82, position: 'relative', marginBottom: 7 },
  ...
  face: { minHeight: 75, borderRadius: 22, borderWidth: 3, borderColor: colors.ink, paddingHorizontal: 13, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 8, overflow: 'hidden' },
