...
type Props = { title: string; eyebrow?: string; onBack: () => void; rightIcon?: AppIconName; onRight?: () => void; rightActive?: boolean; dark?: boolean };
...
export function ScreenHeader({ title, eyebrow, onBack, rightIcon, onRight, rightActive = false, dark = false }: Props) {
  ...
  return (
    <View style={styles.row}>
      ...
      <View style={styles.copy}>
        ...
        <Text numberOfLines={1} style={[styles.title, dark && styles.titleDark]}>{title}</Text>
      </View>
      {rightIcon ? (
        <HeaderButton label="Acțiune" onPress={onRight} dark={dark} active={rightActive}>
